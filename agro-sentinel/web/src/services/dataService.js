 ...
 *
 * In MOCK mode (default): all data comes from dataGenerator.js (no network calls).
 * In API mode: reads from Firestore (live) and Cloud Functions (history + AI).
 *
...
export async function fetchLiveData(locationId) {
    ...
    const res = await fetch(`${SENSOR_API}/live?location=${locationId}`);
...
export async function fetchHistoricalData(locationId, days = 200) {
    ...
    const res = await fetch(`${SENSOR_API}/history?location=${locationId}&days=${days}`);
...
export async function askAI(query, locationId, localData = []) {
    ...
    // Fallback: local analysis on mock data
...
function analyzeLocally(query, locationId, dataset) {
    return new Promise(resolve => {
        setTimeout(() => {
            ...
            if (values.length === 0) return resolve({ answer: 'No hay datos disponibles para esa variable.', sql: '' });
            ...
            if (isAlarm) {
                ...
                if (alarms.length > 0) {
                    const recent = alarms.slice(-5).reverse();
                    const list = recent.map(a => `• ${a.displayDate}: Temp ${a.temp}°C | Hum ${a.humidity}% | CO2 ${a.co2}ppm`).join('\n');
