/**
 * Railway systems of the Americas — researched reference data.
 *
 * Fourteen operating networks, with the companies that build, run and
 * maintain each one, what they cost and who paid. It is the only content in
 * this repository that was gathered rather than generated, and until now it
 * was loaded lazily by the sidebar and rendered by a view no navigation ever
 * reached in normal use: an operator had to expand a collapsed panel, choose
 * a country and click a project. It has its own section now.
 *
 * This is reference material, not telemetry. Nothing here ticks, and nothing
 * here claims to be measured from the fleet.
 */

export type ProjectKind = 'metro' | 'intercity' | 'lrt' | 'cable' | 'suburbano';
export type ProjectStatus = 'operativo' | 'construccion' | 'planificado';

export interface ProjectCompany {
    name: string;
    role: string;
    country: string;
    /** What they supplied, when the role is a supply contract. */
    product?: string;
    /** Contract shape, when the role is an operating or service agreement. */
    contract?: string;
}
...
export interface ProjectLine {
    id: string;
    name: string;
    /** The line's own colour, as published by the operator — a wayfinding
     *  fact about the system, not a choice this design system makes. */
    color: string;
    stops: ProjectStop[];
}
...
export interface RailProject {
    ...
    /** Rolling stock units. Zero where the operator does not publish it. */
    totalTrains: number;
    totalKm: number;
    maxSpeedKmh: number;
    ...
    lines: ProjectLine[];
    companies: {
        operators: ProjectCompany[];
        manufacturers: ProjectCompany[];
        maintenance: ProjectCompany[];
    };
    financing: {
        totalCostUSD: number;
        sources: string[];
...
/** Countries with at least one system on file, alphabetically. */
export const PROJECT_COUNTRIES: string[] = [
    ...new Set(RAIL_PROJECTS.map((project) => project.country)),
].sort((a, b) => a.localeCompare(b, 'es'));

export function projectsByCountry(country: string): RailProject[] {
    return RAIL_PROJECTS.filter((project) => project.country === country);
}

export function projectById(id: string): RailProject | undefined {
    return RAIL_PROJECTS.find((project) => project.id === id);
}
