...
import { Zap, Thermometer, Activity, Settings, MessageSquareText, History, Map as MapIcon, ShieldAlert, LayoutDashboard, Menu, LogOut, ChevronRight, Gauge, Cpu, TrendingUp, AlertTriangle } from 'lucide-react';
...
export default function Dashboard({ onLogout }) {
    ...
    const [selectedMachine, setSelectedMachine] = useState(LOCATIONS[0].id);
    const [historicalData, setHistoricalData] = useState([]);
    const [liveData, setLiveData] = useState([]);

    // Filters: ALL, TEMP, VIB, POWER, OEE
    ...
    useEffect(() => {
        const last24 = generateLast24hData(selectedMachine);
        setLiveData(last24);
        const history = generateHistoricalData(200, 12, selectedMachine);
    ...
    const activeSensors = useMemo(() => {
        return LOCATIONS.map(loc => {
            const isSelected = loc.id === selectedMachine;
            let current = isSelected && liveData.length > 0 ? liveData[liveData.length - 1] : { temp: loc.baseTemp, vpd: loc.baseVib };
            return {
                ...loc,
                temp: current.temp || loc.baseTemp,
                vib: current.vpd || loc.baseVib,
                status: (current.temp > 80 || current.vpd > 5) ? 'ALERTA' : 'OK',
            };
        });
    ...
    const alarmStats = useMemo(() => {
        ...
        let tempCount = 0, vibCount = 0, powerCount = 0;
        const events = [];

        activeData.forEach(d => {
            if (d.temp > 85) {
                tempCount++;
                events.push({ time: d.displayDate || d.time, type: 'SOBRECALENTAMIENTO', value: `${d.temp}°C`, priority: 'CRITICAL' });
            }
            if (d.vpd > 6) { // Vibration > 6 mm/s
                vibCount++;
                events.push({ time: d.displayDate || d.time, type: 'VIBRACIÓN CRÍTICA', value: `${d.vpd} mm/s`, priority: 'CRITICAL' });
            }
            if (d.co2 > 5000) { // Power > 5000W
                powerCount++;
                events.push({ time: d.displayDate || d.time, type: 'SOBRECARGA', value: `${d.co2} W`, priority: 'WARNING' });
            }
            if (d.battery < 50) { // OEE < 50%
                events.push({ time: d.displayDate || d.time, type: 'BAJA PRODUCTIVIDAD', value: `${d.battery}%`, priority: 'WARNING' });
            }
        });
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className={`text-xl font-black leading-none font-mono flex items-baseline gap-1 ${isFieldMode ? 'text-gray-900' : 'text-white'}`}>
                                ...
                                <span className={`text-[10px] font-normal ${isFieldMode ? 'text-gray-400' : 'text-gray-500'}`}>{variable === 'TEMP' ? '°C' : variable === 'VIB' ? 'mm/s' : variable === 'POWER' ? 'W' : '%'}</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className={`flex h-screen font-sans selection:bg-cyan-500/30 selection:text-cyan-900 overflow-hidden transition-colors duration-500 ${isFieldMode ? 'bg-[#f0f2f5] text-gray-900' : 'bg-black text-industrial-100'}`}>
            <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_0%,_#0e749020_0%,_#000000_80%)] pointer-events-none z-0"></div>
            ...
            <aside className={`w-72 bg-industrial-950 border-r border-white/5 flex flex-col z-[70] fixed lg:relative h-full transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                <div className="relative p-8 flex flex-col items-center border-b border-white/5 overflow-hidden">
                    ...
                    <img src={logo} alt="Cognitex" className="h-20 w-auto object-contain drop-shadow-[0_0_15px_rgba(6,182,212,0.3)] mb-4 relative z-10" />
                    <div className="text-center relative z-10">
                        <h2 className="text-xl font-black text-white tracking-tight">INDUSTRY<span className="text-cyan-500">SENTINEL</span></h2>
                        <span className="text-[9px] text-cyan-500/60 tracking-[0.2em] font-mono block mt-1 uppercase">Smart Factory 4.0</span>
                    </div>
                </div>

                <nav className="flex-1 p-4 space-y-6 overflow-y-auto custom-scrollbar relative z-10">
                    <div className="space-y-1">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-4 mb-2">Planta</h3>
                        <button onClick={() => setSelectedVariable('ALL')} className={`w-full group flex items-center justify-between px-4 py-3 rounded-xl text-xs font-bold transition-all duration-300 ${selectedVariable === 'ALL' ? 'bg-gradient-to-r from-cyan-500/20 to-cyan-500/5 text-cyan-400 border border-cyan-500/30 shadow-[0_0_25px_-5px_rgba(6,182,212,0.3)]' : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'}`}>
                            <div className="flex items-center gap-3">
                                <LayoutDashboard size={18} className={selectedVariable === 'ALL' ? 'drop-shadow-[0_0_8px_rgba(6,182,212,0.8)]' : ''} /> DASHBOARD
                            </div>
                        </button>
                    </div>

                    <div className="space-y-1">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-4 mb-2">Variables Críticas</h3>
                        {[
                            { id: 'TEMP', label: 'Temperatura', icon: Thermometer, color: 'text-orange-400' },
                            { id: 'VIB', label: 'Vibración', icon: Activity, color: 'text-purple-400' },
                            { id: 'POWER', label: 'Potencia', icon: Zap, color: 'text-yellow-400' },
                            { id: 'OEE', label: 'OEE / Eficiencia', icon: Gauge, color: 'text-emerald-400' }
                        ].map(item => (
                    ...
                    <div className="space-y-1 pt-2 border-t border-white/5">
                        <h3 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-4 mb-2">Estado Máquinas</h3>
                        <div className="px-4 py-2 space-y-3">
                            <div className="flex items-center justify-between group">
                                <div className="flex items-center gap-3 text-gray-400 text-xs">
                                    <Cpu size={16} className="text-cyan-500" /> Disponibilidad
                                </div>
                                <span className="font-mono text-cyan-400 font-bold bg-cyan-500/10 px-2 py-0.5 rounded text-[10px]">98.2%</span>
                            </div>
                            <div className="flex items-center justify-between group">
                                <div className="flex items-center gap-3 text-gray-400 text-xs">
                                    <Activity size={16} className="text-amber-500" /> Mantenimiento
                                </div>
                                <span className="font-mono font-bold px-2 py-0.5 rounded text-[10px] text-amber-500 bg-amber-500/10">EN 150H</span>
                            </div>
                        </div>
                    </div>
                </nav>
                ...
                <div className="p-4 border-t border-white/5 relative z-10 bg-black/20">
                    <button onClick={onLogout} className="w-full flex items-center justify-center gap-3 px-4 py-4 rounded-xl text-xs font-bold text-gray-400 hover:text-red-400 hover:bg-red-500/5 transition-all border border-white/5 hover:border-red-500/20 group">
                        <LogOut size={16} className="group-hover:-translate-x-1 transition-transform" /> SALIR
                    </button>
                </div>
            </aside>
                ...
                <div className="max-w-[1920px] mx-auto p-6 lg:p-8 space-y-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pb-6 border-b border-white/5">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-3xl font-black text-white tracking-tight mb-1">PANEL INDUSTRIAL</h1>
                                <div className="flex items-center gap-2 text-xs text-gray-400 font-mono">
                                    <span className="w-2 h-2 bg-cyan-500 rounded-full"></span>
                                    Monitoreo IIoT en tiempo real
                                    <span className="text-gray-600">|</span>
                                    {LOCATIONS.find(l => l.id === selectedMachine)?.name}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-3 items-center w-full md:w-auto">
                            <div className="bg-white/5 px-4 py-2 rounded-xl flex items-center gap-3 border border-white/5 flex-1 md:flex-auto hover:border-white/10 transition-colors">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">Línea / Máquina</span>
                                    <select className="bg-transparent text-white font-bold text-sm border-none focus:ring-0 cursor-pointer p-0 w-full md:w-40" value={selectedMachine} onChange={(e) => setSelectedMachine(e.target.value)}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                        {/* Correct mapping from DataGenerator keys: temp, vpd (vibration), co2 (power), battery (oee) */}
                        <KpiCardCompact title="OEE" subtitle="Eficiencia Global" value={stats.battery || '--'} unit="%" icon={<Gauge size={24} />} color="text-emerald-400" bg="bg-emerald-500/10" border="border-emerald-500/20" trend="OPTIMO" />
                        <KpiCardCompact title="VIBRACIÓN" subtitle="RMS Eje Z" value={stats.vpd || '--'} unit="mm/s" icon={<Activity size={24} />} color="text-purple-400" bg="bg-purple-500/10" border="border-purple-500/20" isAlert={stats.vpd > 5} />
                        <KpiCardCompact title="POTENCIA" subtitle="Consumo Real" value={stats.co2 || '--'} unit="W" icon={<Zap size={24} />} color="text-yellow-400" bg="bg-yellow-500/10" border="border-yellow-500/20" />
                        <KpiCardCompact title="TEMP" subtitle="Rodamiento" value={stats.temp || '--'} unit="°C" icon={<Thermometer size={24} />} color="text-orange-400" bg="bg-orange-500/10" border="border-orange-500/20" isAlert={stats.temp > 80} />
                    </div>

                    {selectedVariable === 'ALL' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div className="bg-industrial-900/30 backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex items-center justify-between group hover:border-cyan-500/30 transition-colors relative overflow-hidden">
                                <div className="absolute inset-0 bg-cyan-500/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                <div className="flex items-center gap-4 relative z-10">
                                    <div className="p-3 bg-cyan-500/10 rounded-xl border border-cyan-500/20 text-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.2)]">
                                        <TrendingUp size={24} />
                                    </div>
                                    <div>
                                        <h4 className="text-gray-400 text-[10px] font-extrabold tracking-widest uppercase">Producción Turno</h4>
                                        <p className="text-[10px] text-gray-500">Piezas OK / Total</p>
                                    </div>
                                </div>
                                <div className="text-right relative z-10">
                                    <span className="text-2xl font-black text-white font-mono">1,240</span>
                                    <span className="text-xs text-cyan-500 font-bold ml-1">Pzas</span>
                                    <div className="text-[9px] text-gray-500 mt-1">Meta: 1,500 (+2% vs Ayer)</div>
                                </div>
                            </div>
                            ...
                            <div className="bg-industrial-900/30 backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex items-center justify-between group hover:border-red-500/30 transition-colors relative overflow-hidden">
                                ...
                                <div className="flex items-center gap-4 relative z-10">
                                    ...
                                    <div>
                                        <h4 className="text-gray-400 text-[10px] font-extrabold tracking-widest uppercase">Predicción Fallos</h4>
                                        <p className="text-[10px] text-gray-500">Modelo RUL (AI)</p>
                                    </div>
                                </div>
                                <div className="relative z-10">
                                    <div className="flex items-center gap-2 justify-end">
                                        <div className="h-2 w-24 bg-gray-700 rounded-full overflow-hidden">
                                            <div className="h-full bg-gradient-to-r from-green-500 via-yellow-500 to-red-500 w-[20%]"></div>
                                        </div>
                                        <span className="text-xs font-bold text-green-400">BAJO</span>
                                    </div>
                                    <div className="text-[9px] text-gray-500 mt-1 text-right">Próximo servicio: 340 hrs</div>
                                </div>
                            </div>
                        </div>
                    )}
                            ...
                            {selectedVariable === 'ALL' ? (
                                <>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 min-h-[260px]">
                                        <ChartWidget variable='OEE' title='OEE / EFICIENCIA' color='#10b981' dataKey='battery' />
                                        <ChartWidget variable='VIB' title='VIBRACIÓN (RMS)' color='#a855f7' dataKey='vpd' />
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 min-h-[260px]">
                                        <ChartWidget variable='POWER' title='POTENCIA (W)' color='#eab308' dataKey='co2' />
                                        <ChartWidget variable='TEMP' title='TEMPERATURA' color='#f97316' dataKey='temp' />
                                    </div>
                                </>
                            ) : (
                                <div className="h-full min-h-[500px]">
                                    <ChartWidget variable={selectedVariable} title={selectedVariable} color='#06b6d4'
                                        dataKey={selectedVariable === 'TEMP' ? 'temp' : selectedVariable === 'VIB' ? 'vpd' : selectedVariable === 'POWER' ? 'co2' : 'battery'}
                                    />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="lg:col-span-4 flex flex-col gap-6">
                            <div className="bg-industrial-900/40 backdrop-blur-md border border-white/5 rounded-2xl overflow-hidden min-h-[300px] flex-1 relative flex flex-col hover:border-white/10 transition-colors">
                                <div className="absolute top-4 left-4 z-[400] bg-black/80 px-3 py-1.5 rounded-lg backdrop-blur border border-white/10 shadow-xl">
                                    <h3 className="text-[10px] font-bold text-cyan-400 flex gap-2 items-center tracking-widest uppercase"><MapIcon size={12} /> Digital Twin Map</h3>
                                </div>
                                ...
                                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                    <span className="text-white/20 font-black text-xl">FACTORY LAYOUT</span>
                                </div>
                            </div>
                            ...
                            <div className="bg-industrial-900/40 backdrop-blur-md border border-white/5 rounded-2xl p-6 flex-1 min-h-[300px] flex flex-col hover:border-white/10 transition-colors">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-xs font-bold flex items-center gap-2 text-white tracking-widest uppercase"><ShieldAlert size={16} className="text-red-500" /> Registro Eventos</h3>
                                ...
                                <div className="overflow-y-auto flex-1 custom-scrollbar pr-2 space-y-3">
                                    {alarmStats.recent.length === 0 ? (
                                        <div className="flex flex-col items-center justify-center h-full text-gray-600 gap-2">
                                            ...
                                            <span className="text-xs italic">Sistemas Nominales</span>
                                        </div>
                                    ) : (
